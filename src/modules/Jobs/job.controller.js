...
jobRouter.post("/addJob/:companyId", authentication, authorization(["Hr", "Owner"]), US.addJob);

// 2. Update Job (Only the Job Owner)
jobRouter.put("/updateJob/:jobId", authentication, authorization(["Owner"]), US.updateJob);

// // 3. Delete Job (Only HR or Company Owner)
// jobRouter.delete("/deleteJob/:jobId", authentication, authorization(["HR", "Owner"]), US.deleteJob);

// // 4. Get all Jobs or specific Job (with filters, pagination, sorting)
// jobRouter.get("/getJobs", US.getJobs);
// jobRouter.get("/getJobsByCompany/:companyId/jobs", US.getJobsByCompany);
...
// jobRouter.get("/getJobApplications/:jobId", authentication, authorization(["HR", "Owner"]), US.getJobApplications);

// // 7. Apply to Job (User Role Only)
// jobRouter.post("/applyToJob/:jobId", authentication, authorization(["User"]), US.applyToJob);

// // 8. Accept or Reject an Applicant (Only HR)
// jobRouter.patch("/updateApplicationStatus/:applicationId", authentication, authorization(["HR"]), US.acceptOrRejectApplicant);
